...
public class Task6714 extends OneInputValLayout {
    ...
    protected void makeLayout() {
        appendHeader();
        appendSubheading("Решите задачу " +linkToTask(6589) + " с помощью массивов");
        appendTaskDesc("Пользователь вводит массив чисел через запятую. Первое число обозначает начало 'красного' интервала чисел. Второе число обозначает конец 'красного' интервала чисел. Третье число - начало 'зеленого' интервала чисел. Четвертое - конец 'зеленого' интервала чисел. Интервал считается корректным если для него выполняется условие начало<конец. Нужно определить: пересекаются ли данные два интервала чисел. Если не пересекаются, то написать объяснение.");
