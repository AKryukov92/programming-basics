...
//        }
        ...
        for (int i = 0; i < in7060x1.length; i++) {
            try {
                task7060(in7060x1[i], in7060y1[i], in7060w1[i], in7060h1[i], in7060x2[i], in7060y2[i], in7060w2[i], in7060h2[i], i);
            } catch (IllegalArgumentException ex) {
                System.out.println(ex.getMessage());
            }
        }
    ...
    public static void task7060(int x1, int y1, int w1, int h1, int x2, int y2, int w2, int h2, int testId) {
        ...
        String result = String.format(rectangleString, x1, y1, w1, h1, "red");
        result += String.format(rectangleString, x2, y2, w2, h2, "blue");
